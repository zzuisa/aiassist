...
- 修复前数据核验确认博客库中存在 10 篇 Markdown 内容匹配“女人”的文章。
- 部署后使用真实生产链路验证：roguelife MCP 健康；LLM 路由输出 `query=女人`、`limit=3`；LLM 计划仅包含博客搜索 MCP；工具返回 3 篇文章、总匹配 11 篇。新增的第 11 篇是本复盘文章自身包含测试语句。验证事务已回滚，没有创建 Session、消息或任务记录。
