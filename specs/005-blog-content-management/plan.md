...
- 预览沿用 `markdown-it-py`/前端等价解析规则和安全渲染策略，默认禁用原始 HTML。普通读者视觉增强优先使用受限 `visual-plan` JSON，由 Worker 使用 CJK 字体生成紧凑 PNG 资产，存入私有对象存储并以正文 Markdown 图片插入导语后；技术图继续在隔离容器中只读执行 Mermaid。公式和代码高亮不影响存储文本。
