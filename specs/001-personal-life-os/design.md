...
| `storage` gateway | put/get/delete/sign/metadata | MinIO/local adapters |
...
| `storage-local` | 默认私有对象目录，由 backend 写入、Nginx internal 发送 | named/bind volume；Web Root 外；备份校验 |
...
| 5. 收藏 | 上传、MinIO、缩略图、AI 元数据、瀑布流/详情 | AI 失败仍可看原图；来源标记正确 |
...
| 风险/取舍 | 决策 | 缓解与触发升级条件 |
|---|---|---|
| 组件较多影响个人部署 | 保留需求指定的 PostgreSQL/Redis/RabbitMQ/MinIO | 提供健康、备份、资源下限；小安装可切本地存储但不改变接口 |
