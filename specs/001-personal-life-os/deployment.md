...
> 当前 Codex 工作区运行在 Windows，无法直接访问目标主机的 `/etc/nginx`。本文给出目标主机上可复制
> 执行的备份、修改、验证、重载和回滚步骤。实际修改前必须先登录承载该域名的 Linux 服务器。
...
```text
/etc/nginx/site-available/llm.roguelife.de.conf
```

Debian/Ubuntu 常见目录名称是 `sites-available`。在目标主机先执行以下检查，只选择真实存在的文件：

```bash
if [ -f /etc/nginx/site-available/llm.roguelife.de.conf ]; then
  NGINX_CONF=/etc/nginx/site-available/llm.roguelife.de.conf
elif [ -f /etc/nginx/sites-available/llm.roguelife.de.conf ]; then
  NGINX_CONF=/etc/nginx/sites-available/llm.roguelife.de.conf
else
  echo "llm.roguelife.de.conf 不存在，停止部署" >&2
  exit 1
fi
...
使用 `sudoedit "$NGINX_CONF"` 编辑现有文件。保留原有的 `listen`、`server_name`、证书路径、ACME 和
TLS 配置，不要整份盲目替换。
...
注意：本项目明确使用 `nginx -s reload`，不要在 `nginx -t` 失败时执行 reload。
