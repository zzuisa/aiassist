...
def test_mcp_write_waits_for_confirmation_then_invokes_once(
    ...
    db_session.add(
        McpToolGrant(
            ...
            allowed=True,
            allowed_operations_json=["update"],
            scope_json={},
            granted_at=connection.created_at,
        )
    )
    task = create_agent_task(
        db_session, user_id=user.id, request_text="保存外部笔记", intent_key="mcp.invoke"
    )
    db_session.flush()
    _execute_mcp_task(
        db_session,
        task=task,
        tool_name="mcp.notes-main.notes",
    ...
    decide_pending_write(
        db_session, user_id=user.id, task_id=task.id, confirmation_id=pending.id, decision="approve"
    )
